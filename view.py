...
def tunic_display(temp, scr):
    """
    Function to display server room temperature themed after the indie game, Tunic
    """
    info = pygame.display.Info()
    disp_font = pygame.font.Font('topaz.ttf', 50)
    img = pygame.image.load('tunic.png').convert_alpha()
    width, height = info.current_w, info.current_h
    img = pygame.transform.scale(img, (width, height))     
    scr.blit(img, (0,0))
    message_text = disp_font.render(str(temp) + " degrees Fahrenheit", True, (255, 255, 255))
    message_rect = message_text.get_rect()
    message_rect.center = (width-350,300)
    scr.blit(message_text, message_rect)
